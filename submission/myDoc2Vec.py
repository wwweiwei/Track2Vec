...
class myDoc2Vec(RecModel):
    ...
    def train(self, train_df: pd.DataFrame, **kwargs):
        ...
        documents = [TaggedDocument(doc, [i]) for i, doc in enumerate(p_4.values.tolist())]
        self.mymodel_4 = Doc2Vec(documents, vector_size=self.vector_size, window=self.window, min_count=0, workers=4)
